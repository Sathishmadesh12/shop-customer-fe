  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav">
        ...
        <div className="sep" />
        <button className="nav-item w-full" style={{ border: 'none' }} onClick={handleLogout}>
  ...
  return (
    <header className="header">
      <div className="header-search" style={{ position: 'relative' }}>
        <span className="header-search-icon" style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>🔍</span>
        <input className="header-search-input" placeholder="Search items..." value={search || ''} onChange={(e) => onSearch?.(e.target.value)} />
      </div>
      <div className="header-right">
        ...
        <button className="header-icon-btn" onClick={() => navigate('/cart')}>
          🛒
          {itemCount > 0 && <span className="header-notif-dot">{itemCount}</span>}
        </button>
      </div>
    </header>
  );
...
export const BottomNav = () => {
  ...
  return (
    <nav className="bottom-nav">
      {navs.map((n) => (
        <NavLink key={n.to} to={n.to} end={n.exact} className={({ isActive }) => `bottom-nav-item ${isActive ? 'active' : ''}`}>
          <div className="bottom-nav-icon" style={{ position: 'relative' }}>
            {n.icon}
            {n.badge > 0 && <span className="bottom-cart-badge">{n.badge}</span>}
          </div>
          <span>{n.label}</span>
        </NavLink>
      ))}
